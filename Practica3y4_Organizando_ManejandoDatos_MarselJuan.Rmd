...
```
```{r}
pew_t <- pew_religion %>% gather(key = "income", value="freq", -religion) 
pew_t
```
...
pew_religion_t <- pew_religion %>% gather(key="Income", value="freq", -religion, factor_key=T)
pew_religion_t
#key como quiero que se llame la colkimna
#vlue es la comuna de contendra los bvalores
```
...
```{r}
load("data/billboard.RData")
bill_100 <- billboard %>% pivot_longer(names_to = "week", values_to = "rank", starts_with("wk"), names_prefix = "wk") %>% mutate(data=date.entered)
bill_100
```
...
```{r}
library(dplyr)
billboard_t <- billboard %>% pivot_longer(starts_with("wk"), names_prefix = "wk", names_to="week", values_to="rank") %>% mutate(data=date.entered)
...
tbb <- raw %>% gather(key="m_f", value="casos", -country, -year) %>% separate(col = "m_f", into=c("sexo", "edad"), sep=1)
tbb
```
...
```{r}
load("./data/tb.Rdata")
raw_t <- raw %>% gather(key = "hola", value = "casos", -country, -year)
head(raw_t)

raw_t_t <- raw_t %>% separate("hola", into=c("sexo", "edad"), sep=1) #sep =1 para que separe despues del primer caractyer
head(raw_t_t)
...
hey <- raw %>% pivot_longer(cols=starts_with("d"), names_to = "dia", values_to = "valor", values_drop_na = T) %>% pivot_wider(names_from = "element", values_from = "valor") %>% unite(col= "fecha","year", "month", "dia")
hey
...
planes.f <- planes %>% filter(str_detect(str_to_lower(manufacturer),".*douglas.*")) %>% filter(between(seats, 90, 120)) %>% mutate(manufacturer=ifelse((str_detect(str_to_lower(manufacturer),".*airbus*")), 
                                                                                                                                                        "AIRBUS COMP", 
                                                                                                                                                        manufacturer))
```
...
planes.f <- planes %>%
  #  eliminar aviones de McDonnell Douglas
  filter(!str_detect(str_to_lower(manufacturer), "mcdonnell\\s*douglas")) %>%
  
  # filtrar
  filter(seats >= 90 & seats <= 210) %>%
  

  mutate(manufacturer = ifelse(
    str_detect(str_to_lower(manufacturer), "airbus"),
    "AIRBUS COMP",
    manufacturer
  ))
...
```{r }
flightsm <- flights %>% select(-time_hour, -ends_with("time"))
planesm <- planes %>% select(-speed, -type)
...
Distances_plane <- flights %>%
  filter(!is.na(tailnum)) %>%                     # Eliminar registros con tailnum NA
  group_by(tailnum) %>%                           # Agrupar por avión
  summarise(
